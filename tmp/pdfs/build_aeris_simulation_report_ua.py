...
INPUT_PDF = PROJECT_ROOT / "docs" / "AERIS_Simulation_Report.pdf"
OUTPUT_PDF = PROJECT_ROOT / "docs" / "XPA-105_Simulation_Report_ua.pdf"
IMAGE_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "aeris_sim_full_images"
...
def build_story(style_map):
    ...
    story += [
        ...
        p("AERIS-10 X-діапазонний фазований радарний масив", style_map["cover_subtitle"]),
        ...
        Spacer(1, 1.9 * inch),
        p("AERIS Radar Systems | Березень 2026 | Версія 1.0", style_map["cover_scope"]),
    ...
    story += [
        ...
        p(
            "Цей звіт подає результати повноланцюгової симуляції обробки сигналів для X-діапазонного фазованого радара AERIS-10. Кожен параметр, використаний у моделюванні - центральна частота, смуга chirp-сигналу, частота дискретизації ADC, геометрія антенного масиву, ваги beamforming та часові параметри - був узятий безпосередньо з відкритих файлів апаратного проєкту PLFM_RADAR: Verilog, C firmware, схем і Python GUI.",
            style_map["body"],
        ),
        p(
    ...
    story += [
        ...
        p(
            "Симуляція відтворює вісім етапів обробки, кожен з яких відповідає окремій апаратній підсистемі на платі AERIS-10. Нижче наведено повний ланцюг.",
            style_map["body"],
        ),
        scaled_image("img-000.png", 5.55 * inch),
        p("Рисунок 3.1 - Ланцюг обробки сигналу AERIS-10. Кожен блок відповідає фізичній мікросхемі на радарній платі.", style_map["caption"]),
        make_table(
            style_map,
    ...
    story += fig_page(
        ...
        "Поліпшити роздільну здатність за швидкістю можна, збільшуючи CPI - тобто використовуючи більше chirps на позицію променя або довший PRI. AERIS-10 використовує 32 chirps на промінь, що є компромісом між роздільною здатністю за швидкістю та часом повторного проходу променя.",
        "Апаратна реалізація: доплерівська обробка вимагає фазової когерентності протягом усього CPI (32 chirps x 167 µs = 5.34 ms). Саме тому AERIS-10 використовує OCXO як master reference і має закодований у STM32 firmware 180-second warm-up. Фазовий шум у ланцюгу OCXO -> AD9523-1 -> ADF4382A напряму обмежує мінімальну детектовану швидкість, оскільки повільні цілі маскуються фазошумовими pedestal-рівнями в доплері.",
        small=True,
    )
    story += fig_page(
        ...
        "Коли промінь відводиться від broadside, головний пелюсток ширшає, а підсилення зменшується. На кутах порядку +/-60° array factor уже помітно деградує, тому AERIS-10 обмежує електронне сканування до +/-45° і використовує механічний stepper motor на 200 steps/revolution для повного азимутального покриття.",
    ...
    story += fig_page(
        ...
        "Кутова роздільна здатність визначається апертурою масиву й становить приблизно lambda / (N x d x cos(theta)) ~ 7.2° поблизу broadside. Це добре узгоджується з механічним кроком азимута AERIS-10: 360° / 50 позицій = 7.2°, тобто система дискретизована за кутом практично за критерієм Nyquist.",
    ...
    story += fig_page(
        ...
        "img-016.png",
        5.55 * inch,
        "Рисунок 4.8 - Блок-схема ланцюга обробки сигналу AERIS-10 із позначенням апаратної мікросхеми на кожному етапі.",
    ...
    story += fig_page(
        ...
        None,
        "Апаратна реалізація: ця панель є Python-еквівалентом GUI AERIS-10, для якого у репозиторії PLFM_RADAR є вісім ітерацій. GUI отримує оброблені дані від STM32 через UART і відображає карти дальність-доплер, beam patterns і target detections у реальному часі. Для investor demo ADALM-PHASER також має власний Python GUI на базі PyADI-IIO з дуже схожими живими візуалізаціями.",
        small=True,
    )
    ...
    story += [
        ...
        p(
            "Counter-UAS, тобто виявлення дронів, є головним комерційним ринком для AERIS-10. Цей сценарій моделює реалістичні малі дронові цілі: низький RCS від -15 до +5 dBsm, різні швидкості від 0 до 25 m/s і дальності до 2 km.",
            style_map["body"],
        ),
        make_table(
            style_map,
        ...
        p(
            "Ключовий висновок: в усередненому по елементах режимі без спрямованого beamforming радар обмежений приблизно 500 m для цілі з RCS -10 dBsm. Повний 16-елементний beamforming додає близько 12 dB і розтягує дальність виявлення до 800-1000 m для такого ж RCS. Варіант AERIS-10X із GaN PA QPA2962 на рівні ~33 dBm на елемент просуває це вже до 3+ km.",
            style_map["body"],
        ),
        PageBreak(),
    ]
    ...
    story += fig_page(
        ...
        "Кутова роздільна здатність близько 7° означає, що дві цілі на однаковій дальності, розділені менше ніж на 7° по азимуту, уже не розв'язуються окремо. Для раннього попередження цього достатньо, але для точного трекінгу ні. Саме тому монопульсна оцінка кута в AERIS-10 може уточнювати положення до близько 1° усередині ширини променя.",
    ...
    story += fig_page(
        ...
        "Що це означає для продукту: AERIS-10N без GaN PA має практичну дальність C-UAS виявлення близько 500-800 m для малих дронів. AERIS-10X з GaN PA, які дають ще близько 10 dB TX-потужності, розтягує цей діапазон приблизно до 1.5-3 km. Комерційні конкуренти на кшталт Echodyne EchoGuard досягають 2-3 km на схожій X-band phased-array архітектурі, але в ціні на порядок вище.",
        "Апаратна реалізація: підняти performance можна трьома шляхами: збільшити кількість елементів, подовжити CPI або перейти на GaN PA. Архітектура AERIS-10 підтримує всі три варіанти без редизайну цифрового backend.",
        small=True,
    )
    story += fig_page(
        "5.6 Зведена панель",
        ...
        None,
        "Таку панель можна показувати defense/security інвесторам як доказ того, що AERIS-10 здатний працювати з реалістичними counter-UAS цілями. Поруч із цим логічно демонструвати живий beam steering на ADALM-PHASER при 10.5 GHz.",
        small=True,
    )
    ...
    story += [
        ...
        p(
            "Нижче узагальнено, як кожен вихід симуляції співвідноситься з фізичними компонентами плати AERIS-10, що саме симуляція підтверджує і що вона принципово не може перевірити без реального апаратного тесту.",
            style_map["body"],
        ),
        make_table(
            style_map,
        ...
        Spacer(1, 0.15 * inch),
        p(
            "Правий стовпець цієї таблиці фактично визначає перелік конкретних вимірювань, які потрібно буде зробити на demo-kit ADALM-PHASER і на майбутньому прототипі AERIS-10. Саме ці ризики симуляція сама по собі закрити не здатна.",
            style_map["body"],
        ),
        PageBreak(),
    ]
    ...
    story += [
        ...
        Spacer(1, 0.18 * inch),
        p(
            "AERIS Radar Systems | Технічний звіт про моделювання | Березень 2026 | Усі параметри взято з github.com/NawfalMotii79/PLFM_RADAR (MIT License)\nSimulation code: aeris10_radar_sim.py | PDF generator: generate_sim_report_pdf.py | Environment: Python 3.14 + NumPy + Matplotlib + ReportLab",
            style_map["caption"],
        ),
    ]
