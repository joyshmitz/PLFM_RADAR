...
def build_pages(project_root: Path, image_dir: Path) -> list[Page]:
    ...
    page.text(60, 240, PAGE_W - 120, "AERIS-10 X-діапазонна фазована радарна решітка", 16, color=MID_BLUE, align="center")
    ...
    page.text(70, 548, PAGE_W - 140, "AERIS Radar Systems | Березень 2026 | Версія 1.0", 11, color="#99a3b4", align="center")
    page.text(
        64,
        570,
        PAGE_W - 128,
    ...
    page.paragraph(
        "AERIS-10 використовує Rogers RO4350B - високочастотний термореактивний ламінат, який широко застосовується в X-діапазонних радарах і 5G-вузлах. Основні параметри підкладки було взято зі схем Qucs у репозиторії PLFM_RADAR, калькулятора патч-антени та документації зі стеку плати.",
        size=9.4,
        gap_after=10,
    )
    page.table(
    ...
    page.table(
        ...
        [
            ...
            ["Конфігурація масиву", "8 x 16 (128 елементів)", "Повна специфікація AERIS-10N"],
        ],
        [0.26, 0.22, 0.52],
        font_size=8.4,
        header_font_size=8.7,
    ...
    page.paragraph(
        "Що це показує: комплексний вхідний імпеданс Z_in = R + jX як функцію частоти. У справжньому резонансі уявна частина переходить через нуль, тобто антена є чисто активною, а дійсна частина дорівнює 50 Ohm і збігається з опором тракту. Для патча AERIS-10 отримано 47.1 + j0.2 Ohm на 10.495 GHz - це майже ідеальне узгодження із системою 50 Ohm.",
        size=9.4,
    )
    page.table(
        ["Параметр", "Значення"],
    ...
    page.paragraph(
        "Що це показує: VSWR є альтернативним способом описати якість узгодження імпедансу. VSWR = 1.0 означає ідеальне узгодження, а VSWR < 2.0, що відповідає S11 < -10 dB, є стандартним порогом прийнятної роботи. Патч AERIS-10 має мінімум VSWR близько 1.06 на 10.495 GHz, тобто узгодження є відмінним.",
        size=9.6,
        gap_after=12,
    )
    page.paragraph(
    ...
    page.paragraph(
        "Наслідок для масиву: діаграма одиночного елемента задає оболонку, всередині якої можна електронно сканувати промінь. Ширина пелюстка близько 75-85 deg дозволяє масиву з 128 елементів відхиляти промінь до +/-45 deg без суттєвого падіння підсилення. Після +/-60 deg підсилення елемента швидко зменшується, тому AERIS-10 використовує механічний обертач для повного азимутального покриття.",
        size=9.0,
        gap_after=6,
    )
    page.footer()
    ...
    page.paragraph(
        "Для повного фазованого масиву сімейства AERIS-10N з кроком елементів lambda/2:",
        size=9.4,
        gap_after=8,
    )
    page.table(
    ...
    page.paragraph(
        "Конкурентний контекст: комерційні X-діапазонні фазовані решітки Echodyne EchoGuard і Fortem TrueView зазвичай лежать у діапазоні 25-30 dBi. Оцінене підсилення AERIS-10N на рівні 25.8 dBi є цілком конкурентним і підтверджує правильність вибраної антенної архітектури. Варіант AERIS-10X із підсилювачами потужності на GaN додає ще близько 10 dB по тракту передавання, частково компенсуючи втрати ККД одиночного елемента.",
        size=9.0,
        gap_after=6,
    )
    page.footer()
...
def main() -> None:
    ...
    work_dir = project_root / "tmp" / "pdfs" / "aeris_ua"
